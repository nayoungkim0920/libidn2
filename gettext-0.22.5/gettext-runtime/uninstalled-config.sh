...
if test yes = yes; then
  if test 'yes' = yes; then
    ...
    LIBINTL="${relative_builddir}/intl/.libs/libintl.so -Wl,-rpath,${relative_builddir}/intl/.libs /mingw64/lib/libiconv.dll.a -L/mingw64/lib "
  else
    LIBINTL="${relative_builddir}/intl/.libs/libintl.a /mingw64/lib/libiconv.dll.a -L/mingw64/lib "
  fi
else
...
# gets found.
if test yes = yes; then
  LTLIBINTL="${relative_builddir}/intl/libintl.la -L/mingw64/lib -liconv -R/mingw64/lib "
else
  # The functionality is provided by libc.
  LTLIBINTL=
fi
