...
/* 프로젝트 내부 헤더 */
#include "term_ostream.priv.h"  // term_ostream_representation 등 내부 정의
//#include "term-style-control.h"

/* 파일 관련 헤더 */
#include "term-ostream.h"  // term_ostream_t를 사용하는 헤더

#include "fsync.h"              // fsync 관련 기능
#include "error.h"              // error 처리
#include "full-write.h"         // full-write 관련 함수
#include "get_ppid_of.h"        // 부모 프로세스 ID 얻기
#include "get_progname_of.h"    // 프로그램 이름 얻기
#include "terminfo.h"           // 터미널 정보 관련
#include "xalloc.h"             // 메모리 할당 관련 기능
#include "xmalloca.h"           // 추가된 xmalloca 관련 함수
#include "xgethostname.h"       // 호스트 이름 얻기
#include "xsize.h"              // 사이즈 처리 관련 기능

/* Windows 플랫폼에서 사용할 경우의 추가 헤더 */
#if HAVE_WINDOWS_CONSOLES
# if defined _WIN32 && !defined __CYGWIN__
#  include "msvc-nothrow.h"  // Windows에서 no-throw 처리
# else
#  include <io.h>  // _get_osfhandle() 처리
# endif
#endif
  ...
  if (stream->is_windows_console)
  ...
  #endif
    {
	  ...
	  if (new_attr.hyperlink != NULL){
	    ...
	    out_hyperlink_change ((term_ostream_t)stream, (hyperlink_t *)new_attr.hyperlink, true);
	  }
    }
}
...
activate_default_attr (term_ostream_t stream)
{
  ...
  //deactivate_term_non_default_mode (&controller, (term_style_user_data *) stream->_pointer);
	#if IS_CPLUSPLUS
		deactivate_term_non_default_mode(&controller, (term_style_user_data *) stream->_pointer);
		//deactivate_term_non_default_mode(&controller, convert_to_term_style_user_data((struct term_ostream_representation *) stream)->_pointer);
	#else
		//deactivate_term_non_default_mode(&controller, (term_style_user_data *) stream);
  ...
  if (len > 0)
    {
      if (!equal_attributes (*ap, stream->default_attr)){
				...
				#if IS_CPLUSPLUS
					//activate_term_non_default_mode(&controller, convert_to_term_style_user_data(stream->_pointer));
					activate_term_non_default_mode(&controller, (struct term_ostream_representation *) stream->_pointer);
				#else
